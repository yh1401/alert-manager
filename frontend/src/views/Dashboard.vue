        ...
        <el-row :gutter="16" class="chart-row">
            ...
            <el-col :span="12">
                <el-card shadow="hover" class="chart-card">
                    ...
                    <div ref="syncFailureBarRef" class="chart"></div>
                </el-card>
            </el-col>
        </el-row>
    </div>
...
const syncFailureBarRef = ref(null);
...
const renderCharts = () => {
    ...
    // sync failure bar
    const syncFailureChart = initChart(syncFailureBarRef);
    syncFailureChart?.setOption({
        tooltip: { trigger: "axis" },
        xAxis: {
            type: "category",
            data: stats.sync_failure_stats.map((d) => d.date),
        },
        yAxis: { type: "value" },
        series: [
            {
                name: "失败次数",
                data: stats.sync_failure_stats.map((d) => d.count),
                type: "bar",
...
const fetchAuditStats = async () => {
    ...
    totalActions.value = (stats.action_stats || []).reduce(
        (acc, cur) => acc + (cur.Count || 0),
        0,
    );
    last7Days.value = (stats.daily_stats || []).reduce(
        (acc, cur) => acc + (cur.Count || 0),
        0,
    );
};

const fetchSyncFailureStats = async () => {
    const res = await axios.get("/api/admin/stats/sync_failures");
    stats.sync_failure_stats = res.data?.data || [];
};
